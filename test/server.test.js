...
test("hostAllowed: localhost forms pass, everything else fails", () => {
  assert.equal(srv.hostAllowed({ headers: { host: "localhost:7879" } }), true);
  assert.equal(srv.hostAllowed({ headers: { host: "127.0.0.1:7879" } }), true);
...
test("csrfSafe: POST that is same-origin / none / header-less (curl) is allowed", () => {
  ...
  assert.equal(srv.csrfSafe({ method: "POST", headers: { origin: "http://localhost:7879" } }), true);
});
